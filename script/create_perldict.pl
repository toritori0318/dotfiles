...
my $dictfile = "$ENV{HOME}/.vim/dict/perl_functions.dict";
my @words = ();
my @extwords = qw/for while until foreach if elsif else unless strict warnings /;

while(<DATA>){
   chomp; my $command = (split)[0];
   push @words, $command if $command =~ /^\w\w+$/;
}
push @words, @extwords;
